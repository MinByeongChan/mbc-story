...
URL: [https://matching.coway.com/request](https://matching.coway.com/request)
...
- 기존 Vue.js로 개발된 서비스를 React 환경으로 성공적으로 마이그레이션
- 코드베이스의 복잡도를 낮추고 유지보수성을 향상.

**시스템 안정성 및 정확성 확보**

- 매칭 상태를 확인하는 로직을 FE의 setTimeout 방식에서 BE에서 내려주는 타이머로 제어
- 브라우저 세션 변경과 setTimeout의 부정한 이슈로 현재시간을 비교하는 프로세스(dayjs)를 이용한 타이머 개발
- 매칭 정확성을 높이고 불필요한 API 호출 빈도수 감소

**데이터독 RUM 및 Cypress 연동**

- 데이터독 RUM을 연동하고 Cypress를 활용하여 E2E 테스트 코드를 작성함
- 운영 배포 시 이슈 발생률을 최소화하여 안정적인 배포 실현.

**GA360 활용**

- 기획 및 마케팅팀과의 협업을 통해 GA360 활용하여 마케팅에 필요한 사용자 행동 데이터(이탈률, 제품 정보, 매칭 시도 횟수 등) 수집 및 이슈 대응

**CI/CD 적용**

- CI/CD 파이프라인과 버전관리 및 배포 노트 자동화 구축
- 수동 배포 스크립트를 개발하여 GitHub Publishing을 통해 배포 노트를 기록하고 버전 관리를 체계적으로 설계
...
1. 개발적인 측면에서 성장했습니다.
   ...
   - react-query를 사용하여 비동기 처리, error boundary, react-router-dom v6를 사용하는 등 다양한 기술을 사용하여 사용자가 웹서비스를 이용함에 있어 UI 플리커링 현상이나, 비동기로 인한 비정상적인 동작을 최소한으로 하였습니다.
2. 커뮤니케이션 면에서 성장했습니다.
